...
export function createNav (imgSrcLogo, apiLogout, apiCheckLogIn, homeLink,  shareRecipeLink, loginLink, allrecipelink, searchLink, profileLink, profilePicRoute, aboutLink) {
    ...
    btnLogin.innerHTML = '<ion-icon name="log-in-outline"></ion-icon> <span class="span">Login</span>'
    btnLogin.addEventListener('click',() => logout.logOut(checkLoggedIn.logInStatus, apiLogout, loginLink))
    wrapper.append(searchBtn, uploadBtn, btnLogin ,navtoggleBtn)
...
function reRoute(logInStatus, loginLink, reRouteLink, logInAllData){
    ...
    if(logInStatus === 400){
        ...
        console.log('logout')
        window.location.href = reRouteLink+`/index.html?uid=${logInAllData.uid}`
